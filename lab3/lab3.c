...
void updateWorld()
{
	...
	for (j = i+1; j < kNumBalls; j++){
	    deltaX = VectorSub(ball[i].X, ball[j].X);
	    if((Norm(deltaX) < 2*kBallSize) && (DotProduct(ball[i].v,deltaX) < DotProduct(ball[j].v, deltaX))){
		imp = -(ELASTICITY + 1)*DotProduct(Normalize(deltaX),VectorSub(ball[i].v, ball[j].v))/(1.0/ball[i].mass + 1.0/ball[j].mass);
		ball[i].P = VectorAdd(ScalarMult(Normalize(deltaX), imp), ball[i].P);
		ball[j].P = VectorAdd(ScalarMult(Normalize(deltaX), -imp), ball[j].P);
	    }
	}
    }
...
void init()
{
	...
	ball[i].J = S(ball[i].mass*kBallSize*kBallSize/3,ball[i].mass*kBallSize*kBallSize/3,ball[i].mass*kBallSize*kBallSize/3);
	ball[i].Ji = InvertMat4(ball[i].J);
	ball[i].omega = SetVector(0,0,0);
    }
    ball[0].X = SetVector(0.5, 0, 0);
    ball[1].X = SetVector(0, 0, 0.5);
    ball[2].X = SetVector(0.0, 0, 1.0);
    ball[3].X = SetVector(0.15, 0, 1.5);
    ball[0].P = SetVector(0, 0, 0);
    ball[1].P = SetVector(0, 0, 5);
